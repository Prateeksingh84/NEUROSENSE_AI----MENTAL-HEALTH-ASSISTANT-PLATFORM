...
<img src="static/images/brand/neurosense-logo-256.png" alt="NeuroSense AI Logo" width="120"/>
...
NeuroSense AI is a locally deployable, privacy-first Flask web application that connects **emotional intelligence** with **computational intelligence**. A live webcam detects the user's facial emotion every 1.6 seconds and injects it into every AI response — so the chatbot always knows how you're actually feeling, not just what you're typing.
...
- **MTCNN** (3-stage cascaded CNN) for face detection → **FER CNN** (trained on FER-2013, 35,887 images) for emotion classification
...
- **Web Speech API** (browser-native STT) with support for **16+ language locales**
...
# 5. sql/wellbeing_checkins.sql
# 6. sql/mental_health_templates.sql
# 7. sql/knowledge_chat.sql
# 8. sql/professional_help.sql
# 9. sql/report_types.sql
# 10. sql/seed_admin.sql  ← (optional, sets first admin)
```
...
```bash
# The training CLI runs from the app
python app.py
# Select: Hand Sign Training / CLI option
```

Or run the standalone trainer:
...
|---|---|---|---|
| Hand Sign Accuracy (30 signs) | > 90% | **96.7%** | A+ |
| Voice Recognition Accuracy | > 90% | **95.2%** | A |
| FER — Dominant Emotion | > 85% | **92.9%** | A |
| LLM Response Relevance | > 90% | **96.8%** | A+ |
| End-to-End Response Latency | < 5 sec | **2.3 sec** | A+ |
| User Satisfaction Score | > 4.0 / 5 | **4.7 / 5.0** | A+ |
...
| [`docs/MODELS.MD`](docs/MODELS.MD) | All 10 AI/ML models — specs, parameters, performance |
...
NeuroSense AI contributes to three **United Nations Sustainable Development Goals**:

| SDG | Goal | How NeuroSense AI Contributes |
|---|---|---|
| **SDG 3** | Good Health & Well-being | 24/7 accessible mental health support, crisis detection, professional referral |
| **SDG 4** | Quality Education | Structured mental health education via Research Chat, awareness tools |
| **SDG 10** | Reduced Inequalities | Fully offline-capable, multilingual (50+ languages), sign language accessibility for speech-impaired users |
...
**⚠️ Disclaimer:** NeuroSense AI is a supportive tool and is **not a substitute for professional mental health care.** If you are in crisis, please contact the helplines above.
